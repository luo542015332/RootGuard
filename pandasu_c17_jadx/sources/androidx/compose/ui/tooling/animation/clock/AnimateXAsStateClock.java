...
public final class AnimateXAsStateClock<T, V extends AnimationVector> implements ComposeAnimationClock<AnimateXAsStateComposeAnimation<T, V>, TargetState<T>> {
    ...
    public AnimateXAsStateClock(AnimateXAsStateComposeAnimation<T, V> animation) {
        ...
        this.state = new TargetState<>(getAnimation().m562getAnimationObject().getValue(), getAnimation().m562getAnimationObject().getValue());
    ...
    private final TargetBasedAnimation<T, V> getCurrentAnimation() {
        return AnimationKt.TargetBasedAnimation(getAnimation().getAnimationSpec(), getAnimation().m562getAnimationObject().getTypeConverter(), getState().getInitial(), getState().getTarget(), getAnimation().m562getAnimationObject().getVelocity());
    }
}
