...
public final class InfiniteTransitionClock implements ComposeAnimationClock<InfiniteTransitionComposeAnimation, TargetState<Object>> {
    ...
    public List<ComposeAnimatedProperty> getAnimatedProperties() {
        Iterable $this$mapNotNull$iv = getAnimation().m565getAnimationObject().getAnimations();
    ...
    public long getMaxDurationPerIteration() {
        Long l;
        Iterator it = getAnimation().m565getAnimationObject().getAnimations().iterator();
        if (it.hasNext()) {
    ...
    public List<TransitionInfo> getTransitions(long stepMillis) {
        InfiniteTransition transition = getAnimation().m565getAnimationObject();
    ...
    private final <T, V extends AnimationVector> long getIterationDuration(InfiniteTransition.TransitionAnimationState<T, V> transitionAnimationState) {
        InfiniteRepeatableSpec animationSpec = transitionAnimationState.getAnimationSpec();
        ...
        InfiniteRepeatableSpec repeatableSpec = animationSpec;
        int repeats = repeatableSpec.getRepeatMode() == RepeatMode.Reverse ? 2 : 1;
        VectorizedDurationBasedAnimationSpec animation = repeatableSpec.getAnimation().vectorize(transitionAnimationState.getTypeConverter());
