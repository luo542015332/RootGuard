...
public final class MultiParagraph {
    ...
    private MultiParagraph(MultiParagraphIntrinsics intrinsics, long constraints, int maxLines, boolean ellipsis) {
        ...
        Rect it;
        int m591getMaxHeightimpl;
        Intrinsics.checkNotNullParameter(intrinsics, "intrinsics");
        this.intrinsics = intrinsics;
        this.maxLines = maxLines;
        int i = 0;
        if (!(Constraints.m594getMinWidthimpl(constraints) == 0 && Constraints.m593getMinHeightimpl(constraints) == 0)) {
        ...
        while (index < size) {
            ...
            if (Constraints.m587getHasBoundedHeightimpl(constraints)) {
                m591getMaxHeightimpl = RangesKt.coerceAtLeast(Constraints.m591getMaxHeightimpl(constraints) - ParagraphKt.ceilToInt(currentHeight), i);
            } else {
                m591getMaxHeightimpl = Constraints.m591getMaxHeightimpl(constraints);
            }
            Paragraph paragraph = ParagraphKt.m42Paragraph_EkL_Y(intrinsics2, ConstraintsKt.Constraints$default(0, m592getMaxWidthimpl, 0, m591getMaxHeightimpl, 5, null), this.maxLines - currentLineCount2, ellipsis);
        ...
        this.width = Constraints.m592getMaxWidthimpl(constraints);
    ...
    */
    ...
    public final /* synthetic */ void m33paintRPmYEkk(Canvas canvas, long color, Shadow shadow, TextDecoration decoration) {
        ...
        for (int index$iv = 0; index$iv < size; index$iv++) {
            ...
            it.getParagraph().mo5paintRPmYEkk(canvas, color, shadow, decoration);
    ...
    public final void m32paintLG529CI(Canvas canvas, long color, Shadow shadow, TextDecoration decoration, DrawStyle drawStyle, int blendMode) {
        Intrinsics.checkNotNullParameter(canvas, "canvas");
        canvas.save();
        List $this$fastForEach$iv = this.paragraphInfoList;
        int size = $this$fastForEach$iv.size();
        for (int index$iv = 0; index$iv < size; index$iv++) {
            Object item$iv = $this$fastForEach$iv.get(index$iv);
            ParagraphInfo it = (ParagraphInfo) item$iv;
            it.getParagraph().mo4paintLG529CI(canvas, color, shadow, decoration, drawStyle, blendMode);
            canvas.translate(0.0f, it.getParagraph().getHeight());
        }
        canvas.restore();
    }
    ...
    public final void m34painthn5TExg(Canvas canvas, Brush brush, float alpha, Shadow shadow, TextDecoration decoration, DrawStyle drawStyle, int blendMode) {
        Intrinsics.checkNotNullParameter(canvas, "canvas");
        Intrinsics.checkNotNullParameter(brush, "brush");
        AndroidMultiParagraphDrawKt.m366drawMultiParagraph7AXcY_I(this, canvas, brush, alpha, shadow, decoration, drawStyle, blendMode);
    }
    ...
    public final Path getPathForRange(int start, int end) {
        ...
        for (int i = paragraphIndex; i < size; i++) {
            ...
            if (p.getStartIndex() != p.getEndIndex()) {
                Path.addPath-Uv8p0NA$default(path, p.toGlobal(p.getParagraph().getPathForRange(p.toLocalIndex(start), p.toLocalIndex(end))), 0L, 2, (Object) null);
            }
        }
        return path;
    }
    ...
    public final int m30getOffsetForPositionk4lQ0M(long position) {
        int paragraphIndex;
        if (Offset.getY-impl(position) <= 0.0f) {
            paragraphIndex = 0;
        } else {
            paragraphIndex = Offset.getY-impl(position) >= this.height ? CollectionsKt.getLastIndex(this.paragraphInfoList) : MultiParagraphKt.findParagraphByY(this.paragraphInfoList, Offset.getY-impl(position));
        }
        ...
        return $this$getOffsetForPosition_k_4lQ0M_u24lambda_u2412.toGlobalIndex($this$getOffsetForPosition_k_4lQ0M_u24lambda_u2412.getParagraph().mo2getOffsetForPositionk4lQ0M($this$getOffsetForPosition_k_4lQ0M_u24lambda_u2412.m39toLocalMKHz9U(position)));
    }
    ...
    public final long m31getWordBoundaryjx7JFs(int offset) {
        int paragraphIndex;
        requireIndexInRangeInclusiveEnd(offset);
        if (offset == getAnnotatedString().length()) {
            paragraphIndex = CollectionsKt.getLastIndex(this.paragraphInfoList);
        ...
        return $this$getWordBoundary__jx7JFs_u24lambda_u2417.m38toGlobalGEjPoXI($this$getWordBoundary__jx7JFs_u24lambda_u2417.getParagraph().mo3getWordBoundaryjx7JFs($this$getWordBoundary__jx7JFs_u24lambda_u2417.toLocalIndex(offset)));
    }
