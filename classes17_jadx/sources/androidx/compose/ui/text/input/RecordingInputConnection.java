...
public final class RecordingInputConnection implements InputConnection {
    ...
    public final void updateInputState(TextFieldValue state, InputMethodManager inputMethodManager) {
        ...
        if (this.isActive) {
            ...
            int compositionStart = composition != null ? TextRange.m140getMinimpl(composition.getPackedValue()) : -1;
            TextRange composition2 = state.getComposition();
            int compositionEnd = composition2 != null ? TextRange.m139getMaximpl(composition2.getPackedValue()) : -1;
            inputMethodManager.updateSelection(TextRange.m140getMinimpl(state.getSelection()), TextRange.m139getMaximpl(state.getSelection()), compositionStart, compositionEnd);
        }
    }
    ...
    public CharSequence getSelectedText(int flags) {
        if (TextRange.m136getCollapsedimpl(this.mTextFieldValue.getSelection())) {
            return null;
        }
    ...
    public boolean performEditorAction(int editorAction) {
        ...
        if (applying$iv) {
            switch (editorAction) {
                case 0:
                    imeAction = ImeAction.INSTANCE.m301getDefaulteUduSuo();
                    break;
                case 1:
                default:
                    ...
                    break;
                case 2:
                    imeAction = ImeAction.INSTANCE.m303getGoeUduSuo();
                    break;
                case 3:
                    imeAction = ImeAction.INSTANCE.m307getSearcheUduSuo();
                    break;
                case 4:
                    imeAction = ImeAction.INSTANCE.m308getSendeUduSuo();
                    break;
                case 5:
                    imeAction = ImeAction.INSTANCE.m304getNexteUduSuo();
                    break;
                case 6:
                    imeAction = ImeAction.INSTANCE.m302getDoneeUduSuo();
                    break;
                case 7:
                    imeAction = ImeAction.INSTANCE.m306getPreviouseUduSuo();
                    break;
            }
            this.eventCallback.mo315onImeActionKlQnJC8(imeAction);
            return true;
        }
        return applying$iv;
    }
    ...
    public int getCursorCapsMode(int reqModes) {
        return TextUtils.getCapsMode(this.mTextFieldValue.getText(), TextRange.m140getMinimpl(this.mTextFieldValue.getSelection()), reqModes);
    }
